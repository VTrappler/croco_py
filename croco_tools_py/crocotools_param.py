...
#
CROCOTOOLS_dir = '/home/victor/croco_tools'  #unused
#
#  Run directory
#
RUN_dir = pwd + '/'
...
CROCO_files_dir = '/bettik/trapplev/croco/croco_SA/Run/CROCO_FILES/'
CROCO_files_dir = '/home/victor/croco_dahu2/Run/CROCO_FILES/'
#
#  Global data directory (etopo, coads, datasets download from ftp, etc..)
#
DATADIR = '/home/victor/croco_tools/'
#
...
print('CROCO_files_dir: {}'.format(CROCO_files_dir))

# if ~isOctave
#   eval(['!mkdir -p ',CROCO_files_dir])
# else
#   system(['mkdir -p ',CROCO_files_dir])
# end

# os.makedirs(CROCO_files_dir) # makedirs =? mkdir -p

#
# CROCO file names (grid, forcing, bulk, climatology, initial)
#
grdname = CROCO_files_dir + 'croco_grd.nc'
frcname = CROCO_files_dir + 'croco_frc_M2S2K1.nc'
...
topofile = CROCOTOOLS_dir + '/Topo/etopo2.nc'
#
# ######################################################################
#
...
#
coads_dir = CROCOTOOLS_dir + '/COADS05/'
#
# COADS time (for climatology runs)
#
# coads_time=(15:30:345); # days: middle of each month
coads_time = np.arange(start=15, stop=345 + 30, step=30)
...
#   monthly climatology 
#
woa_time = np.arange(start=15, stop=345 + 30, step=30)
...
#
# qbar_time=[15:30:365];
qbar_time = np.arange(start=15, stop=360, step=30) # days: middle of each month
qbar_cycle=360
#
...
NCEP_version  = 3   # NCEP version: 
                     ...
                     #           1/1/1979 - 31/3/2011
#					      
# Option for using local dataseNtidests (previously downloaded)
...
QSCAT_clim_file  = DATADIR + 'QuikSCAT_clim/' + 'croco_SCOW_month_clim_1999_2009.nc'
# QuikSCAT climatology file# for make_QSCAT_clim.
#
#--------------------------------------------------
...
#######################################################################
ctscope = dir()
ct = dict.fromkeys(ctscope, [])
print('start')
for key in filter(lambda x: not x.startswith('_'), ctscope):
